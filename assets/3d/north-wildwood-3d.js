(function () {
  ...
  function placeFloodForScaleDepthPass() {
    ...
    // outside Middletown Township remain flat map context when Buildings is on.
  ...
  function buildBuildingPopupHtml(properties) {
    var p = properties || {};
    var address = escapeTownAddressHtml(p.address || p.osmName || "Middletown Township building");
  ...
  function addCore3dLayers() {
    ...
    // footprints. Do not extrude basemap context outside Middletown Township.
  ...
  async function ensure3dMap() {
    ...
    glMapPromise = (async function () {
      ...
      glMap.on("error", function (event) {
        console.warn("Middletown Township 3D renderer warning.", event && event.error ? event.error : event);
      });
