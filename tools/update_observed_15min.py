#!/usr/bin/env python3
"""Build/update Middletown Township's compact Sea Bright-gauge water-level archive.
...
NAVD88_OFFSET_FROM_MLLW_FT = -2.13
QUARTER_SECONDS = 15 * 60
FETCH_CHUNK_DAYS = 90
MAX_INTERPOLATION_GAP_SECONDS = 12 * 60 * 60
MAX_ANCHOR_DISTANCE_SECONDS = 12 * 60 * 60
THRESHOLDS_NAVD88 = {"minorLow":3.07,"moderateLow":4.07,"majorLow":5.07}
THRESHOLDS_MLLW = {"minorLow":5.2,"moderateLow":6.2,"majorLow":7.2}
...
def fetch_json(url: str, attempts: int = 4) -> dict:
    ...
    for attempt in range(attempts):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "Middletown-Township-floodmapper-2.0/1.0"})
...
def update_hourly_archive(path: Path, new_days: dict[str, dict], end_date: date) -> None:
    ...
    existing.update(
        {
            "gaugeName": existing.get("gaugeName") or "Sea Bright",
            "site": SITE_ID,
            "parameterCd": PARAMETER_CD,
            "datum": "NAVD88",
